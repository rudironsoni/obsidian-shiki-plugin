...
const browserScript = async content => {
  ...
  const monacoConst = monacoConstNode ? { text: monacoConstNode.textContent, color: getComputedStyle(monacoConstNode).color, style: monacoConstNode.getAttribute('style') } : null;
  return { pluginThemes: { dark: plugin.loadedSettings.darkTheme, light: plugin.loadedSettings.lightTheme }, source: { monacoInSource, fencesVisible, sourceConst, tokenSample: tokenSpans.slice(0, 12) }, monaco: { editorCount: document.querySelectorAll('.monaco-editor').length, monacoConst } };
};
