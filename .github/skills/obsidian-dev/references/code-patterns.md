...
Obsidian renders across multiple OS windows: pop-out leaves, and — since **1.13** — the Settings window. The globals `activeDocument` / `activeWindow` track **whichever window currently has focus**. During a settings `onChange` (or any handler that runs while a pop-out is focused) they point at the _other_ window.
...
class ItemSuggestModal extends SuggestModal<Item> {
	...
	getSuggestions(query: string): Item[] {
		return ALL_ITEMS.filter(item => item.title.toLowerCase().includes(query.toLowerCase()));
	}
